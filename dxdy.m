...
function [dx,dy]=dxdy(dx,dy,rx,ry,dd,sgn,N,theta)

 dx(2)=rx(2,1)+cos(theta(2))*dd(2,1);
 dy(2)=ry(2,1)-sin(theta(2))*dd(2,1);

 for i=2:N-1 %i+1が子．その隣接する点は１とi
        % 隣接点が同じ位置
        if sqrt((rx(i+1,i)-rx(i+1,1))^2+(ry(i+1,i)-ry(i+1,1))^2)==0    
            dx(i+1)=rx(i+1,i)+cos(theta(i+1))*dd(i+1,i);
            dy(i+1)=ry(i+1,i)-sin(theta(i+1))*dd(i+1,i);
        end
